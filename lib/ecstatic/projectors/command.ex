defmodule Ecstatic.Projectors.Command do
  ...
  project %Events.CommandConfigured{} = event, _metadata, fn multi ->
    ...
    Ecto.Multi.insert(multi, :command, command, on_conflict: [set: [handler: event.handler]], conflict_target: [:application_id, :name])
  end

  project %Events.CommandRemoved{} = event, _metadata, fn multi ->
    query = from(c in Command, where: c.application_id == ^event.application_id and c.name == ^event.name)
