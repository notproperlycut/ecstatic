defmodule Ecstatic.Applications.Aggregates.Validators.Entities do
  ...
  def validate_exists(name, application, entity_types) when is_list(entity_types) do
    found =
      Enum.map(entity_types, &validate_exists(name, application, &1))
      |> Validators.any_ok?()
  ...
  def validate_relation(thing, fieldname, application, relation_types)
      when is_list(relation_types) do
    found =
      Enum.map(relation_types, &validate_relation(thing, fieldname, application, &1))
      |> Validators.any_ok?()
    ...
    if found do
      ...
      {:error,
       "#{fieldname} of #{thing.name} (#{relation_name}) was not found among objects of types [#{
         Enum.join(relation_types, ", ")
       }]"}
    end
  end
  ...
  def validate_relation(thing, fieldname, application, relation_type) do
    ...
    case validate_exists(relation_name, application, relation_type) do
      ...
      _ ->
        {:error,
         "#{fieldname} of #{thing.name} (#{relation_name}) was not found among objects of type #{
           relation_type
         }"}
    end
  end
